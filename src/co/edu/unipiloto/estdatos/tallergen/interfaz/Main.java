...
import co.edu.unipiloto.estdatos.tallergen.mundo.Computador.SistemaOp;
...
public class Main <T> {
    ...
    public void agregarCamisa() throws Exception {
        ...
        System.out.println("[1] " + marca.GEF);
        System.out.println("[2] " + marca.NAUTICA);
        System.out.println("[3] " + marca.POLO);
    ...
    public void agregarComputador() throws Exception {
        ...
        SistemaOp[] sistema = SistemaOp.values();

        for (int i = 0; i < sistema.length; i++) {
            System.out.println("[" + (i + 1) + "] " + sistema[i]);
        }

        int indice = Integer.parseInt(br.readLine());
        SistemaOp sistemas = sistema[indice - 1];
        ...
        Producto computador = new Computador(gama, precio, sistemas);
